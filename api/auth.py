...
class PasswordAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Check password using constant-time comparison to reduce timing leakage
        if not secrets.compare_digest(credentials.encode(), self.password.encode()):
            return JSONResponse(
                status_code=401,
...
def check_api_password(
    ...
    # Check password using constant-time comparison to reduce timing leakage
    if not secrets.compare_digest(credentials.credentials.encode(), password.encode()):
        raise HTTPException(
            status_code=401,
