...
MAX_UPLOAD_FILE_SIZE = 500 * 1024 * 1024  # 500 MB
...
def generate_unique_filename(original_filename: str, upload_folder: str) -> str:
    ...
    while True:
        ...
        if not resolved.is_relative_to(file_path.resolve()):
    ...
    try:
        with Path(file_path).open("wb") as f:
            while chunk := await upload_file.read(UPLOAD_CHUNK_SIZE):
                ...
                if total_bytes > MAX_UPLOAD_FILE_SIZE:
                    raise HTTPException(status_code=413, detail="File too large")
                f.write(chunk)

        logger.info(f"Saved uploaded file to: {file_path} ({total_bytes} bytes)")
        return file_path
...
async def create_source(
    ...
    try:
        ...
        if source_data.type == "link":
            ...
            if not file_resolved.is_relative_to(uploads_resolved):
                raise HTTPException(
                    status_code=400,
...
async def download_source_file(source_id: str, view: Optional[str] = None):
    """Download the original file associated with an uploaded source.

    Query parameters:
        view: If set to "inline", serve the file with inline content-disposition
              and proper media type so browsers can render it (e.g. PDF in iframe).
    """
    try:
        resolved_path, filename = await _resolve_source_file(source_id)

        if view == "inline":
            # Determine proper media type for inline viewing
            import mimetypes
